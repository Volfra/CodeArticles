...
import static ws.client.Constraints.inputkey;
import static ws.client.Constraints.outputkey;
import static ws.client.Constraints.paramFile;
...
import data.msa.JABAService;
import data.msa.Metadata;
import data.msa.MsaWS;
import data.msa.RegistryWS;
import data.msa.SequenceAnnotation;
import data.sequence.Alignment;
import data.sequence.FastaSequence;
import data.sequence.ScoreManager;
import data.sequence.SequenceUtil;
import data.sequence.UnknownFileFormatException;
import metadata.JobSubmissionException;
import metadata.Option;
import metadata.Preset;
import metadata.ResultNotAvailableException;
...
public class Jws2Client {
	 ...
	 * @throws ConnectException
	 */
	public static data.msa.RegistryWS connectToRegistry(String host)
			throws WebServiceException, ConnectException {
		URL url = null;
		...
		QName portName = new QName(serv.getServiceName().getNamespaceURI(),
				service + "Port");
		data.msa.RegistryWS serviceIF = serv.getPort(portName,
				data.msa.RegistryWS.class);
